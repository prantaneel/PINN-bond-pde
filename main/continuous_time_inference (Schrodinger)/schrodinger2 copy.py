...
class PhysicsInformedNeuralNetwork:
    def __init__(self, x0, u0, v0, tb, X_f, layers, lb, ub):
        ...
        lb = [-5, 0] #lower bound of [x, t]
        ub = [5, pi / 2] #upper bound of [x, t]
        """
        ...
        self.u0 = tf.Variable(u0, dtype=tf.float32)
        self.v0 = tf.Variable(v0, dtype=tf.float32)
    ...
    def loss_function(self):
        loss = (
            tf.reduce_mean(tf.square(self.u0 - self.u0_pred))
            + tf.reduce_mean(tf.square(self.v0 - self.v0_pred))
    ...
    def predict(self, X_star):
        ...
        u_pred, v_pred, _, _ = self.net_uv(x_pred, t_pred)
        f_u_pred, f_v_pred = self.net_f_uv(x_pred, t_pred)
